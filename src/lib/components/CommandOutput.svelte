		...
		{#each $commandOutput as entry}
			<div class="mb-3 border-b border-border-dim/30 pb-2">
				<div class="flex items-center gap-2 mb-1">
					...
					<span class="text-accent text-[11px] font-semibold">{entry.command}</span>
				...
				{/if}
				{#if entry.error}
					<pre class="text-red whitespace-pre-wrap break-all leading-relaxed pl-4">{entry.error}</pre>
				{/if}
			</div>
		{/each}
	{/if}
